...
def stopping_condition1(z1,v_l_1_list, z2, v_l_2_list,A_inv,theta_est, const, phase):
    ...
    # if (l.T @ A_inv @ l + v_l_norm_max) * const >= l.T @ theta_est + np.exp(-phase*1e-7) * min( np.array(v_l) @ theta_est):
    #     return True
    if (l.T @ A_inv @ l + v_l_norm_max) * const >= l.T @ theta_est + 1/phase * min( np.array(v_l) @ theta_est):
        return True
    else:
        return False
...
#     return z.T @ theta_est - const * z.T @ A_inv @ z
# def stopping_condition(x1,y1_set,x2,y2_set,A_inv,theta_est,const):
#     """
#
#     :param x1: x_star
#     :return:
#     """
#     z1_set = [upper_bound(x1-y1.reshape(DIM,1),theta_est,A_inv,const) for y1 in y1_set]
#     z2_set = [lower_bound(x2 - y2.reshape(DIM,1),theta_est,A_inv,const) for y2 in y2_set]
#
...
class XY_STATIC_ROBUST(object):
    ...
    def check_stop(self):
        ...
        for arm_idx in range(self.K):
            ...
            v_l_set = self.v_l[arm_idx][v_l_set_index]
            ...
            for arm_idx_prime in range(self.K):
                ...
                # try:
                #     if stopping_condition(arm,arm_y,arm_prime,arm_prime_y, self.A_inv,self.theta_hat,const,self.v_l):
                #         stop = False
                #         break
                # except:
                #     stop = False
